...
TIME_OUTPUT=$( (time cat 5mb.json | ./target/release/pretty --iter "$ITER" > /dev/null) 2>&1 )
...
TIME_OUTPUT=$( (time cat 5mb.json | ./target/release/pretty --no-color --iter "$ITER" > /dev/null) 2>&1 )
REAL=$(echo "$TIME_OUTPUT" | grep real | awk '{print $2}')
USER=$(echo "$TIME_OUTPUT" | grep user | awk '{print $2}')
SYS=$(echo "$TIME_OUTPUT" | grep sys | awk '{print $2}')
echo "  user:$USER system: $SYS total: $REAL"
...
TIME_OUTPUT=$( (time cat 5mb.json | ./target/release/pretty --serde --iter "$ITER" > /dev/null) 2>&1 )
REAL=$(echo "$TIME_OUTPUT" | grep real | awk '{print $2}')
USER=$(echo "$TIME_OUTPUT" | grep user | awk '{print $2}')
SYS=$(echo "$TIME_OUTPUT" | grep sys | awk '{print $2}')
echo "  user:$USER system: $SYS total: $REAL"
